...
<script src='<?php echo get_template_directory_uri(); ?>/js/jquery.elevatezoom.js'></script>
   ...
   <?php 
	...
	$post_ll= $wpdb->get_results( "SELECT *  FROM $kw_posts where post_type='post'  order by post_date DESC ");
	...
	?>	
		...
		<h3> 
		<a href="<?=get_option('home')?>/blogdetail/<?=$rr->ID?>">
		   <?php  if(strlen($rr->post_title) > 45) { echo substr($rr->post_title,0,45).'...';} else { echo $rr->post_title;} ?>
		</a>
		</h3>
		<div class="postDate"><?php echo date('M d, Y',strtotime($b_data['post_date'])); ?>
</div>
		<p> 
		 <?php 
		...
		<a href="<?=get_option('home')?>/blogdetail/<?=$rr->ID?>" class="lock-butn"> Read More</a></div>
  <?php 
    }
